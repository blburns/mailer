...
# Postfix Manager - RedHat/CentOS Installation Script
# This script installs and configures the Postfix management application on RedHat-based systems
...
detect_os() {
    ...
    # Check if it's a supported RedHat-based system
    if [[ "$OS" != *"Red Hat"* && "$OS" != *"CentOS"* && "$OS" != *"Rocky"* && "$OS" != *"AlmaLinux"* && "$OS" != *"Fedora"* ]]; then
        print_error "This script is designed for RedHat-based systems only"
        exit 1
    fi
}
...
install_packages() {
    ...
    if command -v dnf &> /dev/null; then
        ...
        dnf install -y postfix postfix-ldap dovecot dovecot-ldap
        
        # LDAP packages
        dnf install -y openldap-servers openldap-clients
        
        # Additional dependencies
        dnf install -y gcc gcc-c++ make openldap-devel cyrus-sasl-devel openssl-devel
        ...
        # Mail server packages
        yum install -y postfix postfix-ldap dovecot dovecot-ldap
        
        # LDAP packages
        yum install -y openldap-servers openldap-clients
        
        # Additional dependencies
        yum install -y gcc gcc-c++ make openldap-devel cyrus-sasl-devel openssl-devel
...
configure_selinux() {
    ...
    if command -v semanage &> /dev/null; then
        ...
        # Allow Postfix to use network
        setsebool -P postfix_can_network_connect on
        
        # Allow Dovecot to use network
        setsebool -P dovecot_can_network_connect on
        
        print_success "SELinux configured successfully"
    else
        print_warning "SELinux management tools not found, skipping SELinux configuration"
    fi
}
...
display_summary() {
    ...
    echo "  4. Review SELinux and firewall settings"
    echo
